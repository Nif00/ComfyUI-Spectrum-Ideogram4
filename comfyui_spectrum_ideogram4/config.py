...
class SpectrumIdeogram4Config:
    ...
    def validate(self) -> "SpectrumIdeogram4Config":
        ...
        if self.bootstrap_first_forecast and (self.degree != 1 or self.warmup_steps > 1):
            raise ValueError("bootstrap_first_forecast requires degree == 1 and warmup_steps <= 1")
        if (
        ...
        if isinstance(self.max_history, bool) or not isinstance(self.max_history, int) or self.max_history < self.min_fit_points:
            raise ValueError(f"max_history must be an integer >= {self.min_fit_points} for degree {self.degree}")
