...
def _as_cpu_float64_vector(value: Any) -> torch.Tensor:
    return torch.as_tensor(value).detach().to(device="cpu", dtype=torch.float64).reshape(-1)


class ForecastRetryActual(RuntimeError):
    """Discard a partial forecast transaction and rerun the solver step natively."""
...
class SpectrumIdeogram4Runtime:
    def __init__(self, config: SpectrumIdeogram4Config):
    ...
    def start_run(
        ...
        *,
        ...
        if self._run is not None:
            raise RuntimeError("Spectrum Ideogram4 runtime already has an active run")
        ...
        schedule_valid = (
            ...
            and math.isfinite(sigma_max)
            and sigma_max > sigma_min
        )
    ...
    def end_run(self, run_id: int) -> None:
        ...
        if self._run.run_id != int(run_id):
            raise RuntimeError("attempted to end a stale Spectrum Ideogram4 run")
        self._step = None
        self._run = None
        self.forecaster.reset()
        self._history_topology = None
    ...
    def coordinate_for_timestep(self, timestep: torch.Tensor | float) -> float:
        if self._run is None:
            raise RuntimeError("Spectrum Ideogram4 runtime is outside a sampling run")
    ...
    def begin_step(self, timestep: torch.Tensor | float) -> dict[str, Any]:
        ...
        if self._step is not None:
            raise RuntimeError("previous Spectrum Ideogram4 solver step was not finalized")
        ...
        if not actual and self._run.max_consecutive_forecasts is not None and self._consecutive_forecasts >= self._run.max_consecutive_forecasts:
            actual, reason = True, "post-forecast sampler refresh"
            advances_window = False
            bootstrap_forecast = False
    ...
    def _require_step(self, run_id: int, step_id: int) -> _StepState:
        if self._run is None or self._run.run_id != int(run_id):
            raise RuntimeError("Spectrum Ideogram4 run context is stale")
        if self._step is None or self._step.step_id != int(step_id):
            raise RuntimeError("Spectrum Ideogram4 solver-step context is stale")
        return self._step
    ...
    def fallback_current_step(self, run_id: int, step_id: int, reason: str) -> None:
        self._fallback_or_retry(self._require_step(run_id, step_id), reason)
    ...
    def begin_model_call(
        ...
        *,
        ...
        normalized_shape = tuple(int(value) for value in expected_shape)
        ...
        if self._history_topology is not None and normalized_topology != self._history_topology:
            self._fallback_or_retry(step, "Ideogram4 packed topology changed within the sampling run")
    ...
    def observe_actual(self, run_id: int, step_id: int, call_id: int, feature: torch.Tensor) -> None:
        step = self._require_step(run_id, step_id)
        call = step.calls[int(call_id)]
        if step.mode != "actual":
            raise RuntimeError("actual Ideogram4 feature observed during a forecast-only step")
        if tuple(feature.shape) != call.expected_shape:
            raise RuntimeError(f"actual Ideogram4 feature shape {tuple(feature.shape)} does not match {call.expected_shape}")
    ...
    def predict(
        ...
        *,
        ...
        if history_shape is None or tuple(call.expected_shape[1:]) != tuple(history_shape[1:]):
            self._fallback_or_retry(step, "Ideogram4 image-token count or hidden width changed")
            return None
        if step.bootstrap_forecast and self.forecaster.history_length != 1:
            self._fallback_or_retry(step, "one-point bootstrap forecast requires exactly one actual history entry")
            return None
        started = time.perf_counter()
        try:
            if step.bootstrap_forecast:
                predicted = self.forecaster.predict_one_point_hold(rows=positions, device=device, dtype=dtype)
            else:
        ...
        if tuple(predicted.shape) != call.expected_shape:
            self._fallback_or_retry(step, "predicted Ideogram4 feature shape is invalid")
            return None
    ...
    def _aggregate_actual(self, step: _StepState) -> torch.Tensor | None:
        ...
        if any(call.topology != topology for call in step.calls):
            self._disable_forecasting("Ideogram4 packed topology changed between model subcalls")
            return None
        ...
        if self._history_topology is None:
            ...
            self._disable_forecasting("Ideogram4 packed topology changed across actual history")
            return None
    ...
    def finalize_step(self, run_id: int, step_id: int) -> None:
        ...
        if not step.calls:
            if step.fallback and self._disabled:
                ...
                self._step = None
                return
            reason = "solver step completed without reaching the native Ideogram4 model wrapper"
            ...
            if newly_disabled:
                LOG.warning(
                    "Spectrum Ideogram4 disabled for the rest of this run because a predict-noise evaluation "
                    "returned without reaching the native Ideogram4 wrapper; accepting the wrapped result as a passthrough."
                )
            return
            ...
            try:
                ...
                if combined is not None and not self._disabled:
                    try:
                        ...
                        self._disable_forecasting(f"actual Ideogram4 feature is incompatible with history: {exc}")
            finally:
    ...
    def abort_step(self, run_id: int, step_id: int) -> None:
        self._require_step(run_id, step_id)
        if self._run is not None and self._run.next_step_id == int(step_id) + 1:
            self._run.next_step_id = int(step_id)
        self._step = None
    ...
    def debug_summary(self) -> str:
        return (
            ...
            f"fallbacks={self.stats.forecast_fallbacks} bypassed_steps={self.stats.bypassed_steps} "
            f"disabled={self.stats.disabled} history_archive_s={self.stats.history_archive_seconds:.3f} "
