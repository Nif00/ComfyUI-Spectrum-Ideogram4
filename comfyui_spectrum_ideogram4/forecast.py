...
class HistoryWeightForecaster:
    """Chebyshev ridge forecasting with model-dtype feature history.

    The regression stores only the small design and Cholesky matrices. Large
    hidden features remain in their model dtype and are streamed in bounded
    chunks during prediction.
    """
    ...
    def persistent_tensor_bytes(self) -> int:
        ...
        tensors.extend(tensor for tensor in (self._design, self._cholesky) if tensor is not None)
    ...
    def update(self, coordinate: float, feature: torch.Tensor, *, take_ownership: bool = False) -> None:
        ...
        if take_ownership and detached.device == storage_device and detached.is_contiguous():
            ...
            archived = detached.to(device=storage_device, dtype=self._feature_dtype, copy=True).contiguous().reshape(-1)
    ...
    def _normalize_rows(self, rows: Sequence[int] | None) -> tuple[int, ...]:
        ...
        resolved = tuple(range(branch_count)) if rows is None else tuple(int(value) for value in rows)
        if not resolved:
    ...
    def predict(
        ...
        *,
        ...
        return self._predict_with_weights(
            self.combined_weights(coordinate, blend_weight),
            rows=rows,
            device=device,
            dtype=dtype,
        )
